...
// 🎧 Telegram File IDs for reliable playback
const MORNING_AZKAR_AUDIO_URL = "CQACAgQAAxkBAAIB1mkSF6YAATuqRMsf6ltsstN7cBF2AgACVBsAAnIgmFCdAp6NN7xkTzYE";
const EVENING_AZKAR_AUDIO_URL = "CQACAgQAAxkBAAIB12kSF8PTTm8Je5x7Q9FR8_xoimVdAAJVGwACciCYUAG5ohcJtejINgQ";
...
bot.onText(/\/mytime/, async (msg) => {
  ...
  if (!user) {
    return bot.sendMessage(chatId, "Please use /start first to set up your timezone.");
  }
...
schedule.scheduleJob("* * * * *", async () => {
  ...
  if (nowUTC.minute() === 0) {
    console.log(`[${nowUTC.format('YYYY-MM-DD HH:mm:ss')} UTC] Checking ${users.length} users`);
  }
  ...
  for (const user of users) {
    try {
      ...
      if ((hour === 7 && minute === 0) || (hour === 17 && minute === 0)) {
        console.log(`🕐 SENDING to user ${user.id} at ${userTime.format('YYYY-MM-DD HH:mm:ss')} ${user.timezone}`);
      }
      ...
      // Morning Azkar at 7:00
      if (hour === 7 && minute === 0) {
        console.log(`☀️ SENDING MORNING AZKAR to ${user.id} in ${user.timezone}`);
        const msg = formatAzkarMessage(morningAzkar, "☀️ Morning Azkar", lang);
        await sendLongMessage(user.id, msg, { parse_mode: "Markdown" });
        await bot.sendAudio(user.id, MORNING_AZKAR_AUDIO_URL, {
          caption: "*Morning Azkar Audio*",
      ...
      // Evening Azkar at 17:00
      if (hour === 17 && minute === 0) {
        console.log(`🌙 SENDING EVENING AZKAR to ${user.id} in ${user.timezone}`);
        const msg = formatAzkarMessage(eveningAzkar, "🌙 Evening Azkar", lang);
        await sendLongMessage(user.id, msg, { parse_mode: "Markdown" });
        await bot.sendAudio(user.id, EVENING_AZKAR_AUDIO_URL, {
          caption: "*Evening Azkar Audio*",
