...
// Define Audio URLs (***REPLACE THESE PLACEHOLDERS*** with your actual audio links)
...
/**
 * Sends a potentially long message by chunking it.
 * This function no longer manages inline keyboards, as the audio will be sent separately.
 */
async function sendLongMessage(chatId, text, options = {}) {
  const chunks = text.match(/[\s\S]{1,4000}/g) || [];
  for (const chunk of chunks) {
    await bot.sendMessage(chatId, chunk, options);
    // Small delay to avoid hitting Telegram rate limits when sending many chunks
    await new Promise((resolve) => setTimeout(resolve, 100));
  }
}
...
// 🧭 Test Command: Sends text then the audio
bot.onText(/\/test/, async (msg) => {
  const chatId = msg.chat.id;
  const now = moment().tz("Africa/Addis_Ababa");
  const user = users.find((u) => u.id === chatId);
...
// =========================
// ⏰ GLOBAL AZKAR SCHEDULE: Sends text then the audio
// =========================
schedule.scheduleJob("* * * * *", async () => {
  const nowUTC = moment.utc();
  for (const user of users) {
